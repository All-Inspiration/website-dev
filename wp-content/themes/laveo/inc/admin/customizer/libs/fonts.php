...
if ( ! function_exists( 'laveo_customizer_get_font_choices' ) ) :
	/**
	 ...
	 */
	function laveo_customizer_get_all_fonts() {
		$heading1       = array( 1 => array( 'label' => sprintf( '--- %s ---', esc_html__( 'Standard Fonts', 'laveo' ) ) ) );
		$standard_fonts = laveo_customizer_get_standard_fonts();
		$heading2       = array( 2 => array( 'label' => sprintf( '--- %s ---', esc_html__( 'Google Fonts', 'laveo' ) ) ) );
		$google_fonts   = laveo_customizer_get_google_fonts();
		...
		return apply_filters( 'laveo_customizer_all_fonts', array_merge( $heading1, $standard_fonts, $heading2, $google_fonts ) );
	}
endif;
...
if ( ! function_exists( 'laveo_customizer_get_font_choices' ) ) :
	 ...
	 * @return array    The fonts in value/label pairs.
	 */
	function laveo_customizer_get_font_choices() {
		$fonts   = laveo_customizer_get_all_fonts();
		$choices = array();
...
if ( ! function_exists( 'laveo_customizer_get_google_font_uri' ) ) :
	/**
	 ...
	 */
	function laveo_customizer_get_google_font_uri( $fonts ) {

		// De-dupe the fonts
		$fonts         = array_unique( $fonts );
		$allowed_fonts = laveo_customizer_get_google_fonts();
		$family        = array();
		...
		foreach ( $fonts as $font ) {
			...
			if ( array_key_exists( $font, $allowed_fonts ) ) {
				...
				$family[] = urlencode( $font . ':' . join( ',', laveo_customizer_choose_google_font_variants( $font, $allowed_fonts[ $font ]['variants'] ) ) );
			}
		}
		...
		$subset = get_theme_mod( 'font-subset', laveo_customizer_get_default( 'font-subset' ) );

		if ( 'all' === $subset ) {
			$subsets_available = laveo_customizer_get_google_font_subsets();
...
if ( ! function_exists( 'laveo_customizer_get_google_font_subsets' ) ) :
	/**
	 ...
	 */
	function laveo_customizer_get_google_font_subsets() {
		return array(
			'all'          => esc_html__( 'All', 'laveo' ),
			'cyrillic'     => esc_html__( 'Cyrillic', 'laveo' ),
			'cyrillic-ext' => esc_html__( 'Cyrillic Extended', 'laveo' ),
			'devanagari'   => esc_html__( 'Devanagari', 'laveo' ),
			'greek'        => esc_html__( 'Greek', 'laveo' ),
			'greek-ext'    => esc_html__( 'Greek Extended', 'laveo' ),
			'khmer'        => esc_html__( 'Khmer', 'laveo' ),
			'latin'        => esc_html__( 'Latin', 'laveo' ),
			'latin-ext'    => esc_html__( 'Latin Extended', 'laveo' ),
			'vietnamese'   => esc_html__( 'Vietnamese', 'laveo' ),
		);
	}
endif;
...
if ( ! function_exists( 'laveo_customizer_choose_google_font_variants' ) ) :
	/**
	...
	function laveo_customizer_choose_google_font_variants( $font, $variants = array() ) {
		$chosen_variants = array();
  		if ( empty( $variants ) ) {
			$fonts = laveo_customizer_get_google_fonts();
		...
		return apply_filters( 'laveo_customizer_font_variants', array_unique( $chosen_variants ), $font, $variants );
	}
endif;

if ( ! function_exists( 'laveo_customizer_get_standard_fonts' ) ) :
	/**
	 ...
	 */
	function laveo_customizer_get_standard_fonts() {
		return array(
			'serif'             => array(
				'label' => _x( 'Serif', 'font style', 'laveo' ),
				'stack' => 'Georgia,Times,"Times New Roman",serif'
			),
			'sans-serif'        => array(
				'label' => _x( 'Sans Serif', 'font style', 'laveo' ),
				'stack' => '"Helvetica Neue",Helvetica,Arial,sans-serif'
			),
			'monospace'         => array(
				'label' => _x( 'Monospaced', 'font style', 'laveo' ),
				'stack' => 'Monaco,"Lucida Sans Typewriter","Lucida Typewriter","Courier New",Courier,monospace'
			),
			'ColaborateRegular' => array(
				'label' => _x( 'ColaborateRegular', 'font style', 'laveo' ),
				'stack' => '"ColaborateRegular", Tahoma, Geneva, sans-serif'
			)
		);
	}
endif;

if ( ! function_exists( 'laveo_customizer_get_font_stack' ) ) :
	/**
	 ...
	 */
	function laveo_customizer_get_font_stack( $font ) {

		$all_fonts = laveo_customizer_get_all_fonts();

		// Sanitize font choice
		$font = laveo_customizer_sanitize_font_choice( $font );
...
if ( ! function_exists( 'laveo_customizer_sanitize_font_choice' ) ) :
	/**
	 ...
	 */
	function laveo_customizer_sanitize_font_choice( $value ) {
		if ( is_int( $value ) ) {
			// The array key is an integer, so the chosen option is a heading, not a real choice
			return '';
		} else if ( array_key_exists( $value, laveo_customizer_get_font_choices() ) ) {
			return $value;
		} else {
			return '';
		}
...
if ( ! function_exists( 'laveo_customizer_get_google_fonts' ) ) :
	/**
	 ...
	 */
	function laveo_customizer_get_google_fonts() {
		return apply_filters( 'laveo_customizer_get_google_fonts', array(
			'ABeeZee'                  => array(
				'label'    => 'ABeeZee',
				'variants' => array(
					'regular',

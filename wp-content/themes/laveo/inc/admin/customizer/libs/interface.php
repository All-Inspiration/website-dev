...
if ( ! function_exists( 'laveo_customizer_register' ) ) : /**
 ...
 * @return void
 */ {
	function laveo_customizer_register( $wp_customize ) {

		$laveo_customizer = laveo_customizer::Instance();

		$options = $laveo_customizer->get_options();
		...
		if ( isset( $options['sections'] ) ) {
			laveo_customizer_add_sections( $options['sections'], $wp_customize );
		}

		// Add the sections
		if ( isset( $options['panels'] ) ) {
			laveo_customizer_add_panels( $options['panels'], $wp_customize );
		}
		...
		foreach ( $options as $option ) {
			...
			if ( isset( $option['type'] ) ) {
				...
				if ( ! isset( $option['sanitize_callback'] ) ) {
					$option['sanitize_callback'] = laveo_customizer_get_sanitization( $option['type'] );
				}
				...
				// Add the setting
				laveo_customizer_add_setting( $option, $wp_customize );
...
add_action( 'customize_register', 'laveo_customizer_register', 100 );
 ...
 * @return void
 */
function laveo_customizer_add_sections( $sections, $wp_customize ) {
 ...
 * @return void
 */
function laveo_customizer_add_panels( $panels, $wp_customize ) {
 ...
 * @return void
 */
function laveo_customizer_add_setting( $option, $wp_customize ) {
 ...
 * @return void
 */
function laveo_customizer_get_sanitization( $type ) {

	if ( 'select' == $type || 'radio' == $type ) {
		return 'laveo_customizer_sanitize_choices';
	}

	if ( 'checkbox' == $type ) {
		return 'laveo_customizer_sanitize_checkbox';
	}
	...
	if ( 'upload' == $type || 'image' == $type ) {
		return 'laveo_customizer_sanitize_file_url';
	}

	if ( 'text' == $type || 'textarea' == $type ) {
		return 'laveo_customizer_sanitize_text';
	}
	...
	if ( 'range' == $type ) {
		return 'laveo_customizer_sanitize_range';
	}
